...
public class CommonAdviceController extends ResponseEntityExceptionHandler {
	...
	public ResponseEntity<BaseResponse> handleDataNotFoundException(DataNotFoundException dataNotFoundException) {
		return ResponseEntity
				.ok(BaseResponse.builder().errorDetails(ERRORS.get(dataNotFoundException.getMessage())).build());
	}

	@Override
	protected ResponseEntity<Object> handleExceptionInternal(Exception ex, Object body, HttpHeaders headers,
			HttpStatus status, WebRequest request) {
		return ResponseEntity.badRequest().body(BaseResponse.builder().errorDetails(
				ErrorDetails.builder().status(BAD_REQUEST).message(ex.getLocalizedMessage()).code("0003").build())
				.build());
	}
	...
	protected ResponseEntity<Object> handleBindException(BindException ex, HttpHeaders headers, HttpStatus status,
		...
		List<ApiValidationError> fieldErrors = ex.getBindingResult().getFieldErrors().stream()
				.map(fieldError -> ApiValidationError.builder().field(fieldError.getField())
		...
		return ResponseEntity.ok().body(BaseResponse.builder()
				.errorDetails(ErrorDetails.builder().status(OK).fieldErrors(fieldErrors).code("0003").build()).build());
	}
	...
	protected ResponseEntity<Object> handleHttpMessageNotReadable(HttpMessageNotReadableException ex,
		...
		return ResponseEntity.badRequest()
				.body(BaseResponse.builder().errorDetails(
						ErrorDetails.builder().status(BAD_REQUEST).message(ex.getMessage()).code("0002").build())
						.build());
	}
}
