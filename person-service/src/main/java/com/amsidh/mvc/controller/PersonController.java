...
public class PersonController {
	...
	public BaseResponse updatePersonById(@PathVariable Integer personId,
			@RequestBody UpdatePersonRequest updatePersonRequest) {
		log.info("Request received for update person by personId {} and person details {}", personId,
				gson.toJson(updatePersonRequest));
	...
	public BaseResponse findPersonById(@PathVariable Integer personId) {
		...
		Optional.ofNullable(addressBaseResponse.getData()).ifPresent(data -> {
			// AddressResponse addressResponse = gson.fromJson(String.valueOf(data),
	...
	public BaseResponse deletePersonById(@PathVariable Integer personId) {
		...
		return BaseResponse.builder().data(String.format("Person with personId %d deleted successfully", personId))
				.build();
	}
