...
public class CustomerBalanceScheduler {
	...
	public void notifyCustomerBalance() {
		personService.getAllPerson().stream().map(PersonResponse::getPersonId).forEach(personId -> {
			...
			log.info("Person with personId {} is having balance amount is {}", personId,
	...
	private Double getAccountBalance(Integer personId) {
		...
		DoubleStream accountBalanceStream = Optional
				.ofNullable(accountServiceFeignClient.getAllAccountByPersonId(personId)).map(baseResponse -> {
					DoubleStream doubleStream = Optional.ofNullable(baseResponse.getData())
							.map(convertData -> objectMapper.convertValue(convertData,
	...
	private Double getAssetBalance(Integer personId) {
		log.info("Calling Account-Service to get balance of all account");
		DoubleStream assetBalanceStream = Optional.ofNullable(assetServiceFeignClient.getAllAssetByPersonId(personId))
				.map(baseResponse -> {
					DoubleStream doubleStream = Optional.ofNullable(baseResponse.getData())
							.map(convertData -> objectMapper.convertValue(convertData,
