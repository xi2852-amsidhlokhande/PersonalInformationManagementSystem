...
public class AssetServiceImpl implements AssetService {
	...
	public AssetResponse saveAsset(AssetRequest assetRequest) {
		...
		return objectMapper.convertValue(assetRepository.save(objectMapper.convertValue(assetRequest, Asset.class)),
				AssetResponse.class);
	}
	...
	public AssetResponse updateAsset(Integer assetId, UpdateAssetRequest updateAssetRequest) {
		...
		}).map(updatedAsset -> objectMapper.convertValue(updatedAsset, AssetResponse.class)).orElseThrow(
				() -> new AssetDataNotFoundException(String.format("Asset with assetId %d not found", assetId)));
	...
	public AssetResponse getAssetById(Integer assetId) {
		...
		return assetRepository.findById(assetId).map(asset -> objectMapper.convertValue(asset, AssetResponse.class))
				.orElseThrow(() -> new AssetDataNotFoundException(
						String.format("Asset with assetId %d not found", assetId)));
	}
